...
def load_example(version: str = CURRENT_VERSION) -> dict:
    return json.loads(
...
def load_translate_example(version: str = CURRENT_VERSION) -> dict:
    return json.loads(
        (_PROMPTS_DIR / version / "translate_example.json").read_text(encoding="utf-8")
    )
...
def build_translate_user_prompt(
    ...
    return (
        ...
        '"knownIssues" in the response you must produce):\n'
